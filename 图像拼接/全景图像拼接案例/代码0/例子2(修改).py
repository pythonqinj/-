...
def Panorama_stitching(image_right, image_left):
    ...
    if len(goodMatch) > 4:
        ...
        # 修正: 确保尺寸正确地从图像对象获取
        right_array = np.array(image_right)
        left_array = np.array(image_left)
        Panorama = cv.warpPerspective(right_array, Homography, (right_array.shape[1] + left_array.shape[1], right_array.shape[0]))
        Panorama[0:left_array.shape[0], 0:left_array.shape[1]] = left_array
        return Panorama
    else:
        print("没有足够的特征点进行拼接。")
        return None
...
if __name__ == '__main__':
    folder_path = r"D:\Pycharm\Code\Intelligent mapping\图像拼接\全景图像拼接案例\场景2"
    print("在文件夹中找到的图片:")
    images = read_images_from_folder(folder_path)
    for img in images:
        print(img)
